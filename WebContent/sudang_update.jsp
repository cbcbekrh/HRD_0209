...
<%
	...
	if(id == null){
		...
		if(rs.next()){
			...
			pstmt=conn.prepareStatement("select * from sudang0209 where id=?");
			pstmt.setString(1, id);
			rs=pstmt.executeQuery();
			if(rs.next()){
				gajok=rs.getInt("gajok");
				gunsok=rs.getInt("gunsok");
				gitasudang=rs.getInt("gitasudang");
				jikchak=rs.getInt("jikchak");
			}
...
%>
<section>
<h1 style="text-align:center;">수당 정보 수정</h1>
...
<tr>
<th>근속수당</th>
<td colspan=3><input id="in" type="text" name="gunsok" value="<%=gunsok %>"></td>
</tr>

<tr>
<th>기타수당</th>
<td colspan=3><input id="in" type="text" name="gitasudang" value="<%=gitasudang %>"></td>
</tr>
...
<script>
function check(){
	form.action="sudang_update_process.jsp";
}
</script>
</html>